...
`
  ...
  {
    id: "mock-workflow-contract",
    slug: "mock-workflow-contract",
    title: "AI Workflow 契约与 Mock 验证",
    type: "methodology",
    problem: "在 AI 产品的早期，如果直接接入复杂的 Dify 真实工作流或高开销 API，会导致前端状态机不稳定、测试反馈链路太长。",
    summary: "通过在 CloudBase 边界上设计 mock workflow 契约，用低成本的静态图片和模拟状态机校验核心路径，先验证用户是否需要商品摆放预览，再推进真实 AI 接入。",
    steps: ["为 AI 交互数据格式定义明确的 JSON Schema 契约", "在前端状态机 and CloudBase 云函数中实现 mock 推理返回", "对低成本用户反馈路径跑通后，再替换为 Dify 或真实 API 节点"],
    relatedProjectIds: ["fitcheck-lab"],
    updatedAt: "2026-06-10",
    contentBody: `
### 01 / 核心判断：用契约隔离复杂 AI 的高开发阻尼

在开发 FitCheck（微信小程序）这类商品摆放预览工具时，最核心的痛点是 AI 图像融合生成（如 Dify 推理流、背景生成接口）的响应延迟高、调试开销大。如果在第一阶段就把前台状态机与真实 AI 工作流高度耦合，任何后端接口的轻微抖动都会阻碍前端 UI 细节与核心路径的调优。

因此，我主张将首版产品限制在图片模式，并用 mock workflow 契约进行解耦。

### 02 / 方法论构建：CloudBase 与 Dify 的边界治理

我们通过在 CloudBase 云函数中定义一套严格的 AI 生成契约格式（Input: 商品图+场景图 URL，Output: 2.5D proxy 摆放坐标与状态标签），在云函数内部模拟（Mock）出推理状态机。

前端只需要稳定调用这一云函数契约，保障了高频图片上传、摆放交互状态切换的流畅性。这就将下一阶段接入 Dify 真实 workflow 时的工程变更限制在了“云函数内部替换接口”，而无需对整个小程序逻辑伤筋动骨。

### 03 / 产品含义：验证核心价值而非工程规模

第一阶段的产品决策是去验证“用户是否确实需要把商品放到自己场景图里看一眼”的简单低成本判断。用 Mock 换取极致的交付速度与稳定性，是 AI 产品经理控制早期试错成本的必备手段。
`
  },
  {
    id: "workflow-decomposition",
    slug: "workflow-decomposition",
    title: "双轨生成与 AI PM 工作流拆解",
    type: "framework",
    problem: "AI 辅助写 PRD 时，直接“一键生成”会导致文档假大空；而纯交互问答又会让追求高产出的 PM 觉得效率低下。",
    summary: "将写 PRD 拆解为“需求采集 - 追问确认 - 结构生成 - 独立评分 - 导出”递进步骤，并设计“全流程模式”与“快速模式”双轨，平衡探索深度与产出速度。",
    steps: ["将 PRD 撰写拆解为包含 Intake 和 Probe 的 5 个步骤", "设计 Dual-Track 生成路径，保留快速产出通道", "为 PRD 设计主客观评分维度并本地保存与导出"],
    relatedProjectIds: ["insightspec-lab"],
    updatedAt: "2026-06-10",
    contentBody: `
### 01 / 核心判断：拆解过程是对抗幻觉的最佳手段

很多 AI PM 辅助工具容易做成“对话框式”的一键 PRD 生成器。但产品经理撰写 PRD 的核心价值不在于快速套用模板，而在于理清需求逻辑。直接一键生成会诱导大模型在缺少背景数据时“编造假设”去补齐格式。

在 InsightSpec 中，我提炼了“双轨生成”和“工作流拆解”设计。将生成拆解为独立步骤，就是为了将 AI 降级为辅助 PM 澄清的工具。

### 02 / 方法论构建：Intake、Probe 与 Dual-Track

我们建立的系统工作流将流程强制拆散：
- **Intake（需求采集）**：收集模糊原始点子；
- **Probe（追问确认）**：识别核心矛盾并提问一个最高杠杆问题；
- **Generate（生成）**：结合澄清证据生成结构化 PRD；
- **Scoring（打分）**：对需求可控性、安全边界做自动化指标评分。

为了兼顾效率与严谨，系统设计了双轨模式：**全流程模式**（Full Workflow）用于模糊想法的高控制发掘，**快速模式**（Quick Mode）用于已有完整草稿的快速审查打分，赋予用户在安全摩擦与效率间的选择权。

### 03 / 产品含义：用交互框架重构“写文档”

写 PRD 的本质是澄清决策。通过双轨模式与阶段解耦，产品不只是一页生成文本的容器，而是成为了指导产品思路、沉淀决策证据的工作台。
`
  },
  {
    id: "plugin-packaging-governance",
    slug: "plugin-packaging-governance",
    title: "方法论的 Codex 插件化封装与受控治理",
    type: "methodology",
    problem: "如何把零散的产品发现方法论和 GitHub 自动修复动作，变成可在 AI Agent 环境（如 Codex）中安全运行、可回滚、可审计的资产？",
    summary: "把方法论和工程脚本封装为带有 dry-run 机制、测试验证证据和 Draft PR 边界的 Codex Plugins，将 AI 的黑盒行为约束在安全的审计框架中。",
    steps: ["使用声明式 SKILL.md 定义边界与阶段规则", "建立 dry-run first 默认规则以避免工具副作用", "设置自动审查门（review gate）阻断不符合 Schema 或测试未过的动作"],
    relatedProjectIds: ["zero-to-one-product-discovery-plugin", "auto-issue-fix-codex-plugin"],
    updatedAt: "2026-06-10",
    contentBody: `
### 01 / 核心判断：从零散脚本到可复用的智能体资产

当我们有了项目方法论（如 Zero-to-One 发现门禁）或者自动代码修复工具（如 Auto Issue Fix）时，如何确保它们能够安全地分发给其他 AI 智能体使用？如果仅靠单纯的 Prompt 解释，模型在多轮会话后会由于上下文遗忘而忽略边界规则。

因此，必须把它们封装成 Codex 平台规范的可插拔插件（Plugins）。

### 02 / 方法论构建：声明式 SKILL.md 与安全沙箱边界

在 Codex 插件封装中，我们通过以下三个核心手段实现受控治理：
- **声明式配置契约 (SKILL.md)**：将发现流程或修复行为解耦为可插拔的指令声明，指定阶段门槛规则和 Schema 合约；
- **Dry-run First (模拟运行优先)**：Auto Issue Fix Plugin 规定默认处于只读或 Dry-run 模拟模式，只有经过用户显式授权和确权，才能进入 live 模式写入代码；
- **Review Gate (自动审查门)**：设置检测验证证据和 Draft PR 边界。修改后必须跑通本地 regression tests 并通过 Auditor 对代码范围的结构化审查评分，最后仅允许创建 Draft PR。

### 03 / 产品含义：打造坚实的 Agent 协作底座

插件封装的重点不是“展示 AI 多么全自动”，而是将高风险的 AI 行为（如改代码、生成PRD）驯服在人类可以审计、回滚、复盘的安全框架（Agent Governance）内。这种高可控性的封装，是实现人机协作（HITL）大规模落地的技术基石。
`
  }
];
